...
app.post('/student',function(req,res){
   ...
   db.student.findAll({where:{index:index}}).then((s)=>{
    ...
    res.json({status:"Ponovo izvrsite unos, nijedno polje ne smije biti prazno!"});
...
app.put('/student/index',function(req,res){ 
    ...
    let grupaStudenta=req.body.grupa; //ovo je grupa koju unosimo za promjenu
        ...
        if(s==undefined){
            ...
            db.grupa.findOrCreate({where:{naziv:grupaStudenta}}).then((g)=>{
                  s.update({grupaId:g[0].id}).then(()=>{
                    return res.json({status:'Promjenjena grupa studentu '+ indeks});
                  });   
                 
            }).catch((ex)=>{
                return res.status(400).json({status:"Pogrešno uneseni parametri!"+ ex.message}); 
             }); 
        } 
    });   
 });
